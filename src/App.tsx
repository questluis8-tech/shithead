  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-green-800 via-green-700 to-green-900 relative overflow-hidden">
      ...
      {(gameState.players.length === 2 || gameState.players.length > 3) && (
        <div className="absolute top-4 xl:top-8 left-1/2 transform -translate-x-1/2">
          ...
          <div className="flex gap-1 xl:gap-2">
            {gameState.players[gameState.players.length === 2 ? 1 : 2]?.faceDownCards.map((_, index) => (
              <Card
                ...
                faceDown={true}
                playerColor="black"
                className="w-10 h-14 xl:w-14 xl:h-20"
              />
            ))}
          </div>
          ...
          <div className="flex gap-1 xl:gap-2">
            {gameState.players[gameState.players.length === 2 ? 1 : 2]?.hand.slice(0, Math.min(6, gameState.players[gameState.players.length === 2 ? 1 : 2]?.hand.length || 0)).map((_, index) => (
              <Card
                ...
                faceDown={true}
                playerColor="black"
                className="w-8 h-12 xl:w-12 xl:h-16"
              />
            ))}
            {(gameState.players[gameState.players.length === 2 ? 1 : 2]?.hand.length || 0) > 6 && (
              <div className="w-8 h-12 xl:w-12 xl:h-16 flex items-center justify-center text-white text-xs bg-black bg-opacity-30 rounded">
                +{(gameState.players[gameState.players.length === 2 ? 1 : 2]?.hand.length || 0) - 6}
              </div>
            )}
          </div>
      ...
      {gameState.players.length === 3 && (
        <div className="absolute left-4 xl:left-12 top-1/2 transform -translate-y-1/2">
        <div className="text-center mb-2">
      ...
      {gameState.players.length === 3 && (
        <div className="absolute right-4 xl:right-12 top-1/2 transform -translate-y-1/2">
        <div className="text-center mb-2">
      ...
      <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2">
        ...
        <div className="flex items-center justify-center gap-4 xl:gap-8 mb-4 xl:mb-8">
          {/* Pile */}
          <div className="text-center">
            <h3 className="text-white font-bold mb-1 xl:mb-2 text-sm xl:text-base">Pile ({gameState.pile.length})</h3>
            <div className="w-16 h-22 xl:w-20 xl:h-28">
              {gameState.pile.length === 0 ? (
                ...
                <div className="relative w-16 h-22 xl:w-20 xl:h-28 overflow-visible">
                  ...
                  {gameState.pile.slice(-3).map((card, index) => (
                    ...
                    >
                      <Card
                        card={card}
                        className="w-16 h-22 xl:w-20 xl:h-28"
                      />
                    </div>
                  ))}
                </div>
          ...
          {/* Deck */}
          <div className="text-center">
            <h3 className="text-white font-bold mb-1 xl:mb-2 text-sm xl:text-base">Deck ({gameState.deck.length})</h3>
            <div className="w-16 h-22 xl:w-20 xl:h-28">
              {gameState.deck.length > 0 ? (
                <Card card={{ suit: 'hearts', rank: 2, id: 'deck-back' }} faceDown={true} className="w-16 h-22 xl:w-20 xl:h-28" />
              ) : (
      ...
      {/* Human Player - Bottom */}
      <div className="absolute bottom-4 xl:bottom-8 left-1/2 transform -translate-x-1/2">
        ...
        <div className="flex flex-col items-center gap-2">
            ...
            <div className="flex gap-1 xl:gap-2">
              {humanPlayer.faceDownCards.map((_, index) => (
                <Card
                  ...
                  className={`w-12 h-18 xl:w-16 xl:h-24 ${
                    humanPlayer.hand.length === 0 && 
                    humanPlayer.faceUpCards.length === 0 && 
                    gameState.currentPlayerIndex === 0 && 
                    gameState.gamePhase === 'playing'
          ...
          <div className="flex gap-1 xl:gap-2">
            ...
            {gameState.gamePhase === 'setup' && Array.from({ length: 3 - humanPlayer.faceUpCards.length }).map((_, index) => (
              <div key={`empty-${index}`} className="w-12 h-18 xl:w-16 xl:h-24 border-2 border-dashed border-gray-400 rounded-lg flex items-center justify-center text-gray-400 text-xs">
                Empty
              </div>
            ))}
          </div>
          ...
          {/* Hand */}
          {humanPlayer.hand.length > 0 && (
            <div className="flex gap-1 xl:gap-2 flex-wrap justify-center max-w-xl xl:max-w-2xl">
